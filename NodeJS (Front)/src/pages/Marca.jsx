...
function Marca() {
  ...
  const cargar = async (nombre = filtroNombre, empresa = filtroEmpresa) => {
    try {
      const res = verActivas
        ? await marcaService.buscarActivas(nombre || null, empresa || null)
        : await marcaService.buscarInactivas(nombre || null, empresa || null)
      setMarcas(res.data)
    } catch {
  ...
  return (
    <div className="-m-6 min-h-screen bg-[#eaf2fb] p-8 text-slate-900">
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 mb-7">
        <div>
          <h1 className="text-4xl font-bold tracking-tight border-l-2 border-slate-800 pl-2">Marcas</h1>
          <p className="text-sm text-slate-500 mt-2">Gestion de marcas y empresas proveedoras</p>
        </div>
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead className="text-slate-800 border-b border-slate-400">
            <tr>
              ...
              <th className="py-4 px-3 text-left font-bold">Empresa</th>
          ...
          <tbody>
            {marcas.length === 0 ? (
              ...
              marcas.map((m) => (
                <tr key={m[0]} className="border-b border-slate-200 hover:bg-white/45">
                  <td className="py-7 px-3 text-slate-500">{m[0]}</td>
                  <td className="py-7 px-3 font-medium">{m[1]}</td>
                  <td className="py-7 px-3">{m[2]}</td>
                  <td className="py-7 px-3">
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => abrirEditar(m)} className="bg-yellow-400 border-2 border-yellow-500 text-slate-950 px-4 py-1.5 rounded-lg text-xs font-bold hover:bg-yellow-300">
                        Editar
                      </button>
                      <button
      ...
      {modalAbierto && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-[#eaf2fb] rounded-md shadow-2xl w-full max-w-xl min-h-[420px] relative p-9">
            <button
            ...
            <h2 className="text-3xl font-bold uppercase mb-9">
              {editando ? 'Editar marca' : 'Registrar nueva marca'}
            </h2>
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="text-slate-400 text-lg">empresa</label>
                <input
                  placeholder="Empresa"
                  value={form.empresa}
